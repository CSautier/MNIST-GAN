...
def create_generator():
    ...
    input = layers.Input(shape=(10,)) #785
...
for e in tqdm(range(100000)):
    ...
    batch=np.concatenate([x_train[np.random.randint(low=xindex[int_to_train],high=xindex[int_to_train+1],size=batch_size)], generator.predict([np.random.normal(0, 1, size=[batch_size,10]), int_generator])])#785
